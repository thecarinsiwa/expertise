    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $currentUserId > 0) {
            ...
            if ($body !== '' && (int) ($_POST['announcement_id'] ?? 0) === $id) {
                if ($parent_comment_id > 0) {
                    ...
                    if ($check->fetch()) {
                        ...
                        header('Location: ' . entity_url($baseUrl, 'announcement', $id, 'comment=1'));
                        exit;
                    }
                } else {
        ...
        if (isset($_POST['set_reaction']) && (int) ($_POST['announcement_id'] ?? 0) === $id) {
            ...
            if (in_array($reactionType, $allowed, true)) {
                ...
                header('Location: ' . entity_url($baseUrl, 'announcement', $id, 'reaction=1'));
                exit;
            }
        }
    }
                    ...
                    <article class="mission-detail-main">
                        ...
                        <div class="announcement-actions mt-4 pt-4 border-top">
                            ...
                            <?php if ($currentUserId > 0): ?>
                                <form method="post" class="d-inline-block announcement-reactions-form" action="<?= entity_url($baseUrl, 'announcement', $id) ?>">
                            ...
                            <?php else: ?>
                                ...
                                <a href="<?= $baseUrl ?>client/login.php?redirect=<?= urlencode(entity_url($baseUrl, 'announcement', $id)) ?>" class="btn btn-sm btn-outline-primary mt-1">Se connecter</a>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="announcement-comments mt-4 pt-4 border-top">
                            ...
                            <?php endif; ?>
                            <?php if ($currentUserId > 0): ?>
                                <form method="post" action="<?= entity_url($baseUrl, 'announcement', $id) ?>" class="mb-4">
                                    <input type="hidden" name="announcement_id" value="<?= $id ?>">
                            ...
                            <?php else: ?>
                                ...
                                <a href="<?= $baseUrl ?>client/login.php?redirect=<?= urlencode(entity_url($baseUrl, 'announcement', $id)) ?>" class="btn btn-sm btn-outline-primary">Se connecter</a>
                            <?php endif; ?>
                                        ...
                                        <li class="announcement-comment-item mb-3 pb-3 border-bottom">
                                            ...
                                            <?php if ($currentUserId > 0): ?>
                                                <div class="mt-2 pt-2">
                                                    ...
                                                    <form method="post" action="<?= entity_url($baseUrl, 'announcement', $id) ?>" class="announcement-reply-form mt-2" id="reply-form-<?= (int) $c->id ?>" style="display:none;">
                                                        <input type="hidden" name="announcement_id" value="<?= $id ?>">
                                                        <input type="hidden" name="add_comment" value="1">
                                ...
                                <ul class="list-unstyled mb-0">
                                    <?php foreach ($recentAnnouncements as $a): ?>
                                        <li class="mb-2 pb-2 border-bottom border-light">
                                            <a href="<?= entity_url($baseUrl, 'announcement', (int) $a->id) ?>" class="text-decoration-none text-dark">
