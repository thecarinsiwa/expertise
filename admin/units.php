...
if ($pdo) {
    ...
    if ($organisationId > 0) {
        ...
        if ($currentOrg) {
            $departments = $pdo->prepare("SELECT d.id, d.name, d.code, d.is_active, d.head_user_id, u.first_name, u.last_name FROM department d LEFT JOIN user u ON d.head_user_id = u.id WHERE d.organisation_id = ? ORDER BY d.name");
            $departments->execute([$organisationId]);
            $departments = $departments->fetchAll();

            $stmtSvc = $pdo->prepare("
                SELECT s.id, s.name, s.code, s.is_active, s.department_id, d.name AS department_name
                FROM service s
            ...
            $stmtUnit = $pdo->prepare("
                SELECT u.id, u.name, u.code, u.is_active, u.service_id, s.name AS service_name, d.name AS department_name
                FROM unit u
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($success || $error) {
            if ($organisationId > 0) {
                ...
                $departments->execute([$organisationId]);
                $departments = $departments->fetchAll();
                $stmtSvc = $pdo->prepare("SELECT s.id, s.name, s.code, s.is_active, s.department_id, d.name AS department_name FROM service s JOIN department d ON s.department_id = d.id WHERE d.organisation_id = ? ORDER BY d.name, s.name");
                $stmtSvc->execute([$organisationId]);
                $services = $stmtSvc->fetchAll();
                $stmtUnit = $pdo->prepare("SELECT u.id, u.name, u.code, u.is_active, u.service_id, s.name AS service_name, d.name AS department_name FROM unit u JOIN service s ON u.service_id = s.id JOIN department d ON s.department_id = d.id WHERE d.organisation_id = ? ORDER BY d.name, s.name, u.name");
                $stmtUnit->execute([$organisationId]);
                $units = $stmtUnit->fetchAll();
            }
        }
...
<?php else: ?>
        ...
        <?php if ($formType === 'department' && ($formAction === 'add' || $editDepartment)): ?>
            <form method="POST" class="mb-4 p-3 bg-light rounded">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-12"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="1"><?= htmlspecialchars($editDepartment->description ?? '') ?></textarea></div>
        ...
        <?php if (count($departments) > 0): ?>
            <div class="table-responsive">
                <table class="admin-table table table-hover table-sm">
                    <thead><tr><th>Nom</th><th>Code</th><th>Responsable</th><th>Statut</th><th class="text-end">Actions</th></tr></thead>
                    <tbody>
    ...
    <div class="admin-card admin-section-card mb-4">
        ...
        <?php if ($formType === 'service' && ($formAction === 'add' || $editService)): ?>
            <form method="POST" class="mb-4 p-3 bg-light rounded">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-12"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="1"><?= htmlspecialchars($editService->description ?? '') ?></textarea></div>
                    <div class="col-12"><button type="submit" class="btn btn-admin-primary btn-sm">Enregistrer</button> <a href="units.php?organisation_id=<?= $organisationId ?>" class="btn btn-secondary btn-sm">Annuler</a></div>
                </div>
            </form>
        <?php endif; ?>
        <?php if (count($services) > 0): ?>
            <div class="table-responsive">
                <table class="admin-table table table-hover table-sm">
                    <thead><tr><th>Nom</th><th>Code</th><th>Département</th><th>Statut</th><th class="text-end">Actions</th></tr></thead>
                    <tbody>
    ...
    <div class="admin-card admin-section-card mb-4">
        ...
        <?php if ($formType === 'unit' && ($formAction === 'add' || $editUnit)): ?>
            <form method="POST" class="mb-4 p-3 bg-light rounded">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-12"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="1"><?= htmlspecialchars($editUnit->description ?? '') ?></textarea></div>
                    <div class="col-12"><button type="submit" class="btn btn-admin-primary btn-sm">Enregistrer</button> <a href="units.php?organisation_id=<?= $organisationId ?>" class="btn btn-secondary btn-sm">Annuler</a></div>
                </div>
            </form>
        <?php endif; ?>
        <?php if (count($units) > 0): ?>
            <div class="table-responsive">
                <table class="admin-table table table-hover table-sm">
                    <thead><tr><th>Nom</th><th>Code</th><th>Service</th><th>Statut</th><th class="text-end">Actions</th></tr></thead>
                    <tbody>
