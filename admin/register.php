<?php
/**
 * Page d'inscription – Administration Expertise
 * Réservée aux superadmins déjà connectés (ou premier utilisateur si table vide).
 */
...
$roleId = null;
if ($pdo) {
    // Cherche le rôle admin système
    $stmt = $pdo->query("SELECT id FROM `role` WHERE `code` = 'admin' AND `is_system` = 1 LIMIT 1");
    $roleRow = $stmt ? $stmt->fetch() : null;
    $roleId = $roleRow ? $roleRow->id : null;

    // Compte les admins existants
    if ($roleId) {
        $cnt = $pdo->prepare("SELECT COUNT(*) FROM `user_role` WHERE `role_id` = ?");
        $cnt->execute([$roleId]);
        $adminExists = (int) $cnt->fetchColumn() > 0;
    }
}
    ...
    if ($firstName === '' || $lastName === '' || $email === '' || $password === '') {
        ...
        if ($check->fetch()) {
            ...
            try {
                ...
                $pdo->commit();
                $success = "Administrateur <strong>" . htmlspecialchars($firstName . ' ' . $lastName) . "</strong> créé avec succès.";
...
<head>
    ...
    <title>Créer un administrateur – Expertise</title>
...
<body>
    ...
    <div class="auth-card">
        ...
        <div class="auth-top">
            ...
            <p>Nouveau compte d'accès au back-office Expertise</p>
            <?php if ($isFirstSetup): ?>
                <div class="badge-setup"><i class="bi bi-star-fill"></i> Configuration initiale</div>
            <?php endif; ?>
        </div>
                ...
                <form method="POST" action="" novalidate id="registerForm">
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="input-group-icon">
                                ...
                                <input type="text" id="last_name" name="last_name"
                                    class="form-control<?= $error ? ' is-invalid' : '' ?>"
                                    value="<?= htmlspecialchars($_POST['last_name'] ?? '') ?>" placeholder="Mukendi"
                    ...
                    <div class="mb-3">
                        <label for="phone" class="form-label">Téléphone <span
                                style="color:var(--c-muted);font-weight:400">(optionnel)</span></label>
                        <div class="input-group-icon">
                    ...
                    <div class="mb-4">
                        <label for="password_confirm" class="form-label">Confirmer le mot de passe <span
                    ...
                    <button type="submit" class="btn-auth" id="submitBtn">
                        <i class="bi bi-person-check-fill me-2"></i>Créer le compte administrateur
                    </button>
                </form>
            <?php endif; ?>
        </div>
